...
void onPaint(HWND hwnd, WPARAM wp, LPARAM lp)
{
	...
	MoveToEx(ps.hdc, r.left+20, ((r.bottom+20)+(r.top+20))/2, NULL);
	LineTo(ps.hdc, r.right - 20, ((r.bottom + 20) + (r.top + 20)) / 2);
	MoveToEx(ps.hdc, ((r.left + 20) + (r.right - 20)) / 2, r.top + 20, NULL);
	LineTo(ps.hdc, ((r.left + 20) + (r.right - 20)) / 2, r.bottom - 20);

	double y;
	int l = ((r.right - 20) - (r.left + 20)) ;
	int h = ((r.bottom - 20) - (r.top + 20)) ;
	MoveToEx(ps.hdc, r.left + 20, ((r.bottom + 20) + (r.top + 20)) / 2, NULL);
	for (double x = 0; x < l; ++x)
		LineTo(ps.hdc, 20 + x, 89); // dorzucic mapowanie y
